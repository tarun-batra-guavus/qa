...
for each in roles_list_job:
    ...
    for ip in nodes_type:
        node = connect(ip)
        node.setMode("shell")
        status = node.checkAzkaMaster(ip)
        if status == "true":
            running_jobs = node.getRunningJobId(project_name)
            running_jobs_id = running_jobs[0::2]
            running_jobs_name = running_jobs[1::2]
            node.stopRunningJobs(ip,running_jobs_id)
        else:
            continue
...
for each in roles_list_job:
    ...
    for ip in nodes_type:
        ...
        status = node.checkAzkaMaster(ip)
        if status == "true":
            node.stopRunningJobs(project_name,running_jobs_name)
        else:
            continue
...
for each in list(set(all_unique_roles)):
    ...
    for ip in nodes_type:
        node = connect(ip)
        node.setMode("shell")
        node.backup_create_repos(summary_handle,each,common["image_url"],common["mode"].upper(),summary_var_dict)
...
#### Restore backed-up /etc/yum.repos.d to original location
for each in list(set(all_unique_roles)):
    nodes_type = get_nodes_by_type(each)
    for ip in nodes_type:
        node = connect(ip)
        node.setMode("shell")
        node.restore_repos(summary_handle,each,common["mode"].upper(),summary_var_dict)
