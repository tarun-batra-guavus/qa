...
for node_alias in kafkanodes:
    ...
    if int(flag) == len(kafkanodes):
        logger.info("all Kafka's brokers are listed in Confluent Kafka")
      	...
      	logger.info("Not all Kafka's brokers are listed on node %s" %node_alias)
    
    logger.info("Reloading node %s"%node_alias)
    node.reboot()
    logger.info("Checking Kalfa Brokers on %s node after reboot"%node_alias)
    flag = node.kafkaBrokers(node_alias)
    if int(flag) == len(kafkanodes):
        logger.info("all Kafka's brokers are listed on %s node after HA"%node_alias)
    else:
        report.fail("Not all Kafka's brokers are not listed on node %s after HA" %node_alias)
