...
def connect(node_alias, force_new=False):
    ...
    from GMSNode import GMSMixin
    from TMNode import TMMixin
    ...
    for node_type in node_types:
        ...
        if node_type == "NAMENODE":
            ...
            Mixin(NodeClass, GMSMixin)
        elif node_type == "TM":
            Mixin(NodeClass, TMMixin)
        elif node_type :
            Mixin(NodeClass, TMMixin)
        else:
...
def get_db_dir(*args):
    ...
    nodes = []
    for type in args:
	for k, v in testbed.iteritems():
		if type.upper() in v["type"]:
			if "shellpath" in v:
				dir = v["shellpath"]
				print "dir is " , dir
        logger.debug("DB directory for %s node: %s" % ( type, dir))
    return dir
...
def find_standby(nodes):
    """Find the Standby TM node among a list of nodes"""
    nodes = map(connect_if_required, nodes)
    for node in nodes:
        if node.isStandby():
            logger.debug("%s is TM standby" % node)
            return node

    logger.error("Could not find any standby from the cluster")
    return None
