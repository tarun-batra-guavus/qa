...
def parse_file(filename):
        ...
        try:
            with open(filename) as input:
			 ...
			 line = line.strip()
                         tmpList = line.split(",")
                         #list_roles.insert(tmpList[2])
                         config_dict[tmpList[0]] = tmpList[1]
            return(config_dict)
        except:
             logger.error("IO Error while opening file with name %s" % (filename))
             raise IOError("IO Error while opening file with name %s" % (filename))
...
def csv_diff(csv1_file, csv2_file, delimiter=",", error_percent_plus=1, error_percent_minus=1, key_columns=[1]):
    ...
    with open(csv1_file, "rU") as csv1_fd:
        for line in csv1_fd:
            ...
            if "itrate" in line or "rowth" in line  or "imestamp" in line or  "Total" in line or "Traffic" in line  or  not line:
                continue
            ...
            values = [float(val) for idx, val in enumerate(fields, start=1) if idx not in key_columns]
    ...
    with open(csv2_file, "rU") as csv2_fd:
        for line in csv2_fd:
            ...
            for csv1_val, csv2_val in zip(csv1_values, csv2_values):
                if  abs(csv1_val - csv2_val) > 10000 :
                    if csv1_val != 0 :
                        difference = (csv1_val - csv2_val) * 100 / csv1_val
                    elif csv2_val != 0 :
                        difference = (csv1_val - csv2_val) * 100 / csv2_val
                    else:
                        difference = 0
                else:
                    difference = 0
    ...
    for k, v in csv1_dict.iteritems():
        diff_str += "-%s\n" % v[1]
    return diff_str
def diffxml(filename1,filename2,ignorefile):
	file1 = filename1
	file2 = filename2
	difffile = "/var/www/html/useroutput/hbase/diff.txt"
	ignore = ignorefile
	f1 = open(file1,"r")
	f2 = open(file2, "r")
	f3 = open(difffile,"w+")
	...
	for i in list3:
        	i = i.strip()
        	final_list.append(i)
	logger.info("ignore parm list is %s"  %final_list)
	while counter < len(list1):
        	line=list1[counter].strip("\n")
        	if "<name>" in line:
                	param1=line.split("name>")[1].split("<")[0].strip()
                	if param1 in final_list:
                        	counter = counter + 2
                	else:
                        	if list1[counter] == list2[counter]:
                                	counter = counter + 1
                        	else:
                                	f3.write("Difference in line number:" + str(counter) + '\n' + "value in file 1:" + list1[counter] + "value is file 2:" + list2[counter])
                                	counter = counter + 1
        	else:
                	if list1[counter] == list2[counter]:
                        	print "line matched"
                        	...
                        	f3.write("Difference in line number:" + str(counter) + "value in file 1:" + list1[counter] + "value is file 2:" + list2[counter])
                        	counter = counter + 1
	return(difffile)	
...
def replace_variables_in_template(template, variables, output_file=None):
	...
	logger.exception(str(variables))
