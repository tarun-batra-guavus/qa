...
######################################################
#
# Netflow v9 support added by sandeep.nanda@guavus.com
#
######################################################
Netflow9Template_ipv4 = [
       { 'Length' : 4, 'Id' : 8, 'Name' : 'src_addr'  }, # SRC_ADDR
       { 'Length' : 4, 'Id' : 12, 'Name' : 'dst_addr' }, # DST_ADDR
       { 'Length' : 1, 'Id' : 9, 'Name' : 'src_mask'  }, # SRC_MASK
       { 'Length' : 1, 'Id' : 13, 'Name' : 'dst_mask' }, # DST_MASK
       { 'Length' : 4, 'Id' : 15, 'Name' : 'next_hop' }, # NEXT-HOP
       { 'Length' : 2, 'Id' : 10, 'Name' : 'input_iface' }, # INPUT
       { 'Length' : 2, 'Id' : 14, 'Name' : 'output_iface' }, # OUTPUT
       { 'Length' : 4, 'Id' : 2, 'Name' : 'pkts_sent'  }, # PKTS
       { 'Length' : 4, 'Id' : 1, 'Name' : 'bytes_sent'  }, # BYTES
       { 'Length' : 4, 'Id' : 22, 'Name' : 'start_time' }, # FIRST
       { 'Length' : 4, 'Id' : 21, 'Name' : 'end_time' }, # LAST
       { 'Length' : 2, 'Id' : 7, 'Name' : 'src_port'  }, # SRC_PORT 
       { 'Length' : 2, 'Id' : 11, 'Name' : 'dst_port' }, # DST_PORT
       { 'Length' : 1, 'Id' : 6, 'Name' : 'tcp_flags'  }, # FLAGS
       { 'Length' : 1, 'Id' : 4, 'Name' : 'ip_proto'  }, # PROT
       { 'Length' : 1, 'Id' : 5, 'Name' : 'tos'  }, # TOS
       { 'Length' : 2, 'Id' : 16, 'Name' : 'src_as' }, # SRC_AS
       { 'Length' : 2, 'Id' : 17, 'Name' : 'dst_as' }, # DST_AS
       { 'Length' : 3, 'Id' : 70, 'Name' : 'mpls_label_1' }, # MPLS_LABEL_1 3 - MPLS LABEL
       { 'Length' : 3, 'Id' : 71, 'Name' : 'mpls_label_2' }, # MPLS_LABEL_2 3 - S=1, VPN LABEL
       { 'Length' : 1, 'Id' : 46, 'Name' : 'mpls_top_label_type' }, # MPLS_TOP_LABEL_TYPE
       { 'Length' : 4, 'Id' : 47, 'Name' : 'mpls_top_label_addr' } # MPLS_TOP_LABEL_ADDR
]

Netflow9Template_ipv6 = [
       { 'Length' : 16, 'Id' : 27, 'Name' : 'src_addr_v6'  }, # IPv6 SRC_ADDR
       { 'Length' : 16, 'Id' : 28, 'Name' : 'dst_addr_v6' }, # IPv6 DST_ADDR
       { 'Length' : 1, 'Id' : 29, 'Name' : 'src_mask_v6'  }, # IPv6 SRC_MASK
       { 'Length' : 1, 'Id' : 30, 'Name' : 'dst_mask_v6' }, # IPv6 DST_MASK
       { 'Length' : 4, 'Id' : 15, 'Name' : 'next_hop' }, # NEXT-HOP
       { 'Length' : 2, 'Id' : 10, 'Name' : 'input_iface' }, # INPUT
       { 'Length' : 2, 'Id' : 14, 'Name' : 'output_iface' }, # OUTPUT
       { 'Length' : 4, 'Id' : 2, 'Name' : 'pkts_sent'  }, # PKTS
...
class Netflow9(NetflowBase):
    ...
    __hdr__ = (
        ...
        ('sys_uptime', 'I', 0),
        ('unix_sec', 'I', 0),
        ('package_sequence', 'I', 0),
        ('source_id', 'I', 0),
    )

    class NetflowFlowset(NetflowBase):
        __hdr__ = (
            ('flowset_id', 'H', 0), # <= 255 for Template Flowset, > 255 for Data Flowset
            ('length', 'H', 0),     # size of complete flowset
        )

        def __init__(self, *args, **kwargs):
            template = kwargs.pop("template", None)
            super(Netflow9.NetflowFlowset, self).__init__(*args, **kwargs)
            if template != None:
                self.flowset_id = template.template_id

        def __str__(self):
            self.length = self.__hdr_len__ + sum(len(d) for d in self.data)
            return self.pack_hdr() + ''.join(map(str, self.data))

    class NetflowTemplate(NetflowBase.NetflowRecordBase):
        __template__ = Netflow9Template_ipv4
        __hdr__ = [
            ('template_id', 'H', 0),
            ('field_count', 'H', len(__template__)),
        ]

        for f in __template__:
            __hdr__.append((f["Name"], 'H', f["Id"]))
            __hdr__.append((f["Name"] + "_len", 'H', f["Length"]))
            
    class NetflowTemplate_ipv6(NetflowBase.NetflowRecordBase):
        __template__ = Netflow9Template_ipv6
        __hdr__ = [
            ('template_id', 'H', 0),
            ('field_count', 'H', len(__template__)),
        ]
    ...
    class NetflowRecord(NetflowBase.NetflowRecordBase):
        ...
        # Add fields to NetflowRecord Header
        for f in __template__:
            __hdr__.append((f["Name"], FORMAT_STRING[f["Length"]], 0))

        def __str__(self):
            mpls_label_1 = self.mpls_label_1
            mpls_label_2 = self.mpls_label_2

            # If mpls_top_label_type is not set explicitly by caller
            if not self.mpls_top_label_type:
                if mpls_label_1:
                    self.mpls_top_label_type = 0x05     # LDP
                elif mpls_label_2:
                    self.mpls_top_label_type = 0x03     # VPN

            # Encode Mpls Label in separate bytes
            # 20-bits-mpls-label, 3-exp-bits, 1-end-of-stack
            self.mpls_label_1 = ((mpls_label_1 >> 12) & 0xFF, (mpls_label_1 >> 4) & 0xFF, (mpls_label_1 & 0x0F) << 4 | 0)
            self.mpls_label_2 = ((mpls_label_2 >> 12) & 0xFF, (mpls_label_2 >> 4) & 0xFF, (mpls_label_2 & 0x0F) << 4 | 1)   # bottom-of-stack

            return super(Netflow9.NetflowRecord, self).__str__()

    class NetflowRecord_ipv6(NetflowBase.NetflowRecordBase):
        """Netflow v9 flow record."""
        __template__ = Netflow9Template_ipv6
        __hdr__ = []
        ...
        def __str__(self):
            mpls_label_1 = self.mpls_label_1
            mpls_label_2 = self.mpls_label_2
            src_addr_v6 = self.src_addr_v6
            dst_addr_v6 = self.dst_addr_v6
            ...
            # Encode v6 addresses in a 2-tuple
            self.src_addr_v6    = ((src_addr_v6 >> 64) & MAX_INT64, src_addr_v6 & MAX_INT64)
            self.dst_addr_v6    = ((dst_addr_v6 >> 64) & MAX_INT64, dst_addr_v6 & MAX_INT64)

            return super(Netflow9.NetflowRecord_ipv6, self).__str__()
