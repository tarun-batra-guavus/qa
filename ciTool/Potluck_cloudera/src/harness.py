...
file_name = "userinput/config.txt"
if os.path.exists(file_name):
    with open(file_name) as input:
        for line in input:
            if line.startswith("image_url"):
                tmpList = line.split("=")
                build_path = tmpList[1]
                break
...
parser.add_option("-a", "--toolmode", dest="toolmode",
                  help="HR for regression and healthcheck in integrated mode,UH for upgrade and healthcheck,ALL for upgrade,healthcheck and regression")
parser.add_option("-f", "--failuremode", dest="failuremode",
                  help="Failuremode yes/no")
...
if options.dbid is not None:
    ...
    if not options.suite and not options.shell and not options.script and not options.mode:
        harness_logger.error("Suite or Script or mode is a mandatory argument. Use 'harness.py --help'")
        parser.print_help()
        exit(1)
...
report.WEB_URL = "http://%s/Potluck_cloudera/logs/%s" % (MACHINE_IP, relative_logdir_path)
...
try:
    ...
    report.print_report(report_file, MailTo,build_path)


'''Code for Running Auto Upgrade and Healthcheck in integrated mode'''
if options.toolmode == 'UH':
    report_file_txt = report_file + ".txt"
    fail_counter = obj_parser.checkreport(report_file_txt)
    if fail_counter == 0:	    	
	print "Starting healthcheck...."
    	os.system("./harness -e " + options.mail_to + " -s Testsuites/CARE/healthcheck.xml -t" + options.testbed)
    elif options.failuremode == 'y':
	print "###Upgrade failed on setup.But user has selected to continue even if upgrade fails.So starting healthcheck###"
	print "Starting healthcheck...."
    	os.system("./harness -e " + options.mail_to + " -s Testsuites/CARE/healthcheck.xml -t" + options.testbed)
    else:
	print "###Upgrade failed on setup.User has selected to exit if upgrade fails.So,Healthcheck cannot be run###"

'''Code for Running Healthcheck and Regression tool in integrated mode'''
if options.toolmode == 'HR':
    report_file_txt = report_file + ".txt"
    fail_counter = obj_parser.checkreport(report_file_txt)
    if fail_counter == 0:	    	
	...
	print "### Healthcheck failed on setup.But user has selected to continue even if healthcheck fails.So starting regression run###"
	print "Starting regression tool...."
	os.chdir("Golden_Data_Validation_Framework/Test_Driver")
    	os.system("python driver.py")
    else:
	print "### Healthcheck failed on setup.User has selected to exit if healthcheck fails.So,Regression cannot be run###"



if options.toolmode == 'ALL':
    report_file_txt = report_file + ".txt"
    fail_counter = obj_parser.checkreport(report_file_txt)
    healthcheck_suite= "Testsuites/CARE/healthcheck.xml"
    #healthcheck_suite= "Testsuites/CARE/test.xml"
    
    if fail_counter == 0: 	    	
	os.system("./harness -e " + options.mail_to + " -s " + healthcheck_suite + "  -t" + options.testbed +  " -a regression" + " -f y")
    elif options.failuremode == 'y':
	print "### Upgrade failed on setup.But user has selected to continue even if upgrade fails.So starting healthcheck ###"
        os.system("./harness -e " + options.mail_to + " -s " + healthcheck_suite + "  -t" + options.testbed +  " -a regression" + " -f y")
    else:
...
if options.toolmode == 'regression':
        #report_file_new = report_file_txt.replace("upgrade",healthcheck_suite.rstrip(".xml").split("/")[2])
        report_file_new = report_file + ".txt"
        fail_counter = 0
        fail_counter = obj_parser.checkreport(report_file_new)
        if fail_counter == 0:
                print "#######Starting Regression tool########"
                os.chdir("Golden_Data_Validation_Framework/Test_Driver")
                os.system("python driver.py")
